...
class LigerFusedLinearPPOBase(torch.autograd.Function):
    ...
    def _compute_chunk_loss(
        ...
        log_probs, _ = LigerFusedLinearPPOBase.chunk_forward(input_chunk, selected_token_ids_chunk, weight, bias=bias, temperature=temperature)
        ...
        if use_ref_model and ref_per_token_logps_chunk is None:
            with torch.no_grad():
                ref_log_probs, _ = LigerFusedLinearPPOBase.chunk_forward(
                    ref_input_chunk, selected_token_ids_chunk, ref_weight, bias=ref_bias, temperature=temperature
                )
    ...
    @staticmethod
    def chunk_forward(input_chunk, selected_token_ids_chunk, weight, bias=None, temperature=1.0):
        ...
        log_probs = selective_log_softmax(logits, selected_token_ids_chunk)
